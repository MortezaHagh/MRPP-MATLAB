function [Model, GV, Open, Robot, isComplete, nodesToUpdate] = adaptedDstarLite_dir(Model, GV, Open , Robot, isComplete, nodesToUpdate)
...
% new start node
dTheta = turnCost(Robot.Start.nodeNumber, sucNodes, Model, Robot.dir);
[~, sortedInds] = sortrows([Robot.G(sucNodes)+Model.Successors{Robot.Start.nodeNumber,2}(ocpRobotIds); abs(dTheta); rand(1,numel(sucNodes))]');
newStartNode = sucNodes(sortedInds(1));
newDir = Robot.dir+dTheta(sortedInds(1));
...
if Robot.Start.nodeNumber~=Robot.sGoal
    ...
    while s~=Robot.sGoal && numel(nodes)<Model.occLength
        ...
        dTheta = turnCost(s, sucNodes, Model, tempDir);
        ...
        [sortCost, sortedInds] = sortrows([Robot.G(sucNodes) + Model.Successors{s,2}(indsucNodes); abs(dTheta); rand(1,numel(sucNodes))]');
        newStartNode = sucNodes(sortedInds(1));
        tempDir = tempDir+dTheta(sortedInds(1));
